...
static void runTS(LADSPA_Handle instance, unsigned long sample_count, snd_seq_event_t * events, unsigned long EventCount);
...
static void connectPortTS(LADSPA_Handle instance, unsigned long port, LADSPA_Data * data) {
    TS *plugin;
...
static LADSPA_Handle instantiateTS(const LADSPA_Descriptor * descriptor, unsigned long s_rate) {
    unsigned int i;

    TS *plugin_data = (TS *) malloc(sizeof(TS));
    for (i = 0; i < MIDI_NOTES; i++) {
        plugin_data->omega[i] = M_PI * 2.0 / (double) s_rate * pow(2.0, (i - 69.0) / 12.0);
    }
...
static void runTSWrapper(LADSPA_Handle instance, unsigned long sample_count) {
    runTS(instance, sample_count, NULL, 0);
}

static void runTS(LADSPA_Handle instance, unsigned long sample_count, snd_seq_event_t *events, unsigned long event_count) {
    TS *plugin_data = (TS *) instance;
    ...
    for (pos = 0, event_pos = 0; pos < sample_count; pos++) {
        ...
        while (event_pos < event_count && pos == events[event_pos].time.tick) {
            ...
            if (events[event_pos].type == SND_SEQ_EVENT_NOTEON) {
                ...
                data[events[event_pos].data.note.note].active = events[event_pos].data.note.velocity > 0;
                data[events[event_pos].data.note.note].phase = 0.0;
            }
            else if (events[event_pos].type == SND_SEQ_EVENT_NOTEOFF) {
                data[events[event_pos].data.note.note].active = 0;
            }
            event_pos++;
        }
...
void __attribute__ ((constructor)) opened() {
    char **port_names;
...
void __attribute__ ((destructor)) closing() {
    if (tsLDescriptor) {
